...
\begin{algorithm}[ht]
  ...
  \If{$\text{lat} > \text{max\_latency}$}{
      $\text{lat} \leftarrow \text{max\_latency}$ \tcp*{Cap latency to maximum value}
  }
  
  \If{$\text{tp} > \text{max\_throughput}$}{
      $\text{tp} \leftarrow \text{max\_throughput}$ \tcp*{Cap throughput to maximum value}
  }
...
The reward calculation algorithm (Algorithm \ref{algo:reward_calculation}) takes statistics from the NVM Middleware and computes the reward based on certain criteria.
